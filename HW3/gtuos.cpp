...
uint64_t GTUOS::handleCall(const CPU8080 & cpu) {
	...
	switch(cpu.state->a) {
		...
		case GTUOS::FILE_CREATE :	cycleCount = OperationFileCreate(cpu); break;
		case GTUOS::FILE_CLOSE : 	cycleCount = OperationFileClose(cpu); break;
		case GTUOS::FILE_OPEN :		cycleCount = OperationFileOpen(cpu); break;
		case GTUOS::FILE_READ :		cycleCount = OperationFileRead(cpu); break;
		case GTUOS::FILE_WRITE :	cycleCount = OperationFileWrite(cpu); break;
		case GTUOS::FILE_SEEK :		cycleCount = OperationFileSeek(cpu); break;
		case GTUOS::DIR_READ :		cycleCount = OperationDirRead(cpu); break;
...
int GTUOS::OperationFileCreate(const CPU8080 &cpu) {
	const int cycle = 50;
	uint16_t address;
	...
	while(cpu.memory->at(address) != '\0') {
		name.push_back(cpu.memory->at(address++));
	}	
...
int GTUOS::OperationFileOpen(const CPU8080 &cpu) {
	const int cycle = 50;
	uint16_t address;
	std::vector<char> name;
	int status;
	...
	status = fileSystem.OpenFile(name);
	for (int i=0; i<openFiles.size(); ++i) {
		if (openFiles[i] == status) {
			found = true;
		}
	}
...
int GTUOS::OperationFileWrite(const CPU8080 &cpu) {
	const int cycle = 50;
	bool found = false;
	std::vector<char> data;
	uint16_t address;
	...
	if (found) {
		...
		while(index < cpu.state->d) {
			char buffer = (char)cpu.memory->at(address+index);
